...
class Patient_empty_card extends StatelessWidget {


  final double? width;
  final double? height;
  final VerifyPatinetList? PatientDetails;
 // final VerifyOtpResponseOTP PatientData;
  final int? pos;

  const Patient_empty_card({
    Key? key,
    this.width,
    this.height,
    this.PatientDetails,
  ...
  Widget build(BuildContext context) {
    ...
    print('Sarav preet singh ++++++======> ${PatientDetails}');
    return new GestureDetector(
      onTap: () async {
        saveData(PatientDetails!);
        final prefs = await SharedPreferences.getInstance();
          prefs.setString('Patient_Length', "2");
