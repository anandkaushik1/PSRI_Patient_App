...
class Teleempty_card extends StatelessWidget {
  ...
  Future<void> _ConformationOfAppointment(
     ...
     if(CommonStrAndKey.isTeleConsultFlg)
      {
        msg=
        'Do you want to book Tele Consultation Appointment';
      }else
        {
          msg=
          ...
          actions: <Widget>[
            FlatButton(
              ...
              onPressed: () {
                ...
                Navigator.of(context).pop();
                bookAppointment(data, DoctorId, dateStr, remarksController.text, 'Pay Now');
              },
            ),
            FlatButton(
              ...
              onPressed: () {
                if (!CommonStrAndKey.isTeleConsultFlg) {
                  Navigator.of(context).pop();
                  bookAppointment(data, DoctorId, dateStr, remarksController.text, "Pay Later");
                }else{
                  Navigator.of(context).pop();
                }
              },
  ...
  Future<void> _bookedAppointment(BuildContext context,
      GetDoctorAppointmentSlotJsonListArray data,
      String DoctorId,
      DateTime dateStr) async {
      return showDialog<void>(
  ...
  showAlerDialogConform(
    ...
    if(CommonStrAndKey.isTeleConsultFlg)
    {
       message = 'Do you want to book Tele Consultation Appointment' +
          '\n' +
          convertDateToStringFromatForView(dateStr) +
          " " +
          data.appointmentTimeAmPm!.toString();
    }else {
       message = 'Do you want to book Appointment' +
          '\n' +
          convertDateToStringFromatForView(dateStr) +
          " " +
          data.appointmentTimeAmPm!.toString();
  ...
  Future bookAppointment(GetDoctorAppointmentSlotJsonListArray data,
      String DoctorId, DateTime dateStrr, String remarksData, payOption) async {
    String url = BasicUrl.baseUrl;
    final myService = MyServicePost.create(url);
