...
import 'package:flutter_patient_app/Tele_Book_Appointment_pack/TeleDoctorList_Only_screen.dart';

class TeleSpecialisation_card extends StatefulWidget {
  double? width;
  double? height;
  SpecialisationDoctorListArray? SpecialisationData;
  int? pos;

  TeleSpecialisation_card(
...
class _SpecialisationCardState extends State<TeleSpecialisation_card> {
  double? width;
  double? height;
  SpecialisationDoctorListArray? SpecialisationData;
  int? pos;
    ...
    return InkWell(
      onTap: (){
        Navigator.push(  context,
            MaterialPageRoute(
                builder: (context) => TeleDoctorList_Only_screen(
  ...
  Widget getDocImage() {
    if(SpecialisationData!.specialisationImagePath!.isEmpty){
      ...
      return CircleAvatar(
        radius: 30.0,
        backgroundImage:
        NetworkImage(SpecialisationData!.specialisationImagePath!),
